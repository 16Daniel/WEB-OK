...
@Component({
  ...
  styleUrls: ['./inventario-art-semanal.component.css']
})
export class InventarioArtSemanalComponent implements OnInit {
    ...
    getReporte(ciudad, dateInit) {
      ...
      const fechaStr = dateInit.toString();
      const [year, month, day] = fechaStr.split("-").map(Number);
      const fecha = new Date(year, month - 1, day); 
       if (fecha.getDay() > 0 ) {
       alert("Solo se puede seleccionar los dias Domingo")
        return
      }
      this.carga = true;
      this.service
        .serviceGeneralGet(`Dashboard/performance-reporte-ArtSem/${ciudad}/?initDate=${dateInit}`)
        .subscribe((resp) => {
          if (resp.success) {
